...
export function ProjectCard({ repo, viewMode }: ProjectCardProps) {
  ...
  return (
    <motion.div
      ...
      className="h-full"
    >
      <Link href={`/projects/${repo.name}`} className="block h-full outline-none">
        <Card className={`group relative flex h-full overflow-hidden transition-all duration-300 hover:border-primary hover:shadow-[6px_6px_0px_0px_hsl(var(--primary))] ${isList ? "flex-col md:flex-row items-center" : "flex-col"}`}>
          ...
          <CardHeader className={`relative z-10 w-full ${isList ? "md:w-1/3" : ""}`}>
          ...
          <CardContent className={`relative z-10 w-full flex-grow ${isList ? "md:w-2/3 md:pt-6" : ""}`}>
            ...
            <div className="flex flex-wrap gap-2">
              {repo.topics?.slice(0, 3).map((topic) => (
                <Badge key={topic} variant="secondary" className="text-[10px] font-mono px-1.5 py-0">
                  {topic}
                </Badge>
              ))}
          ...
          <CardFooter className={`relative z-10 w-full flex items-center justify-between border-t border-border/50 bg-muted/20 px-6 py-4 ${isList ? "md:w-auto md:border-t-0 md:border-l md:flex-col md:justify-center md:gap-2" : ""}`}>
            {repo.language && (
